#[cfg(feature = "sqlite")]
pub mod sqlite {
    use crate::app::tasks::{BoxTaskVec, Task, TaskStatus};
    ...
    use sqlx::{
        ...
        query, query_as,
        sqlite::{
            Sqlite, SqliteArgumentValue, SqlitePool, SqlitePoolOptions, SqliteRow, SqliteTypeInfo,
        },
        Database, Decode, Encode, Error, FromRow, Row, Type, TypeInfo,
    };
    ...
    impl SQLiteStore {
        ...
        async fn fill_subtasks<'a>(&'a self, task: &'a Task, pool: &'a SqlitePool) -> BoxTaskVec {
            ...
            match raw_subtasks_query.await {
                ...
                Err(_) => {
                    println!("Couldn't retrieve subtasks for task #{}", task.id)
                }
            };
            results
        }
        async fn read_orphans(&self, pool: &SqlitePool) -> anyhow::Result<BoxTaskVec> {
            let orphans: Vec<Box<Task>> =
                query_as("SELECT * FROM tasks WHERE parent_task_id ISNULL;")
                    .fetch_all(pool)
                    .await?
                    .into_iter()
                    .map(Box::new)
        ...
        async fn write_tasks_helper(
            ...
            for task in tasks {
                ...
                let _ = query("REPLACE INTO tasks VALUES (?,?,?,?,?);")
                    ...
                    .await;
                let _ = self
                    .write_tasks_helper(pool, task.get_subtasks(), Some(task.id))
                    .await;
            }
            Ok(())
        }
    ...
    where
        i32: Decode<'r, DB>,
    {
        fn decode(value: <DB as Database>::ValueRef<'r>) -> Result<TaskStatus, BoxDynError> {
